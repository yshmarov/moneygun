...
module UserHelper
  def user_avatar(user, classes: "w-8 h-8")
    if user.identities.any? && user.identities.first.image_url.present?
      image_tag user.identities.first.image_url, alt: user.identities.first.name, class: [classes, "rounded-full"].compact
    else
