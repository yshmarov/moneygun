...
The one-pager deliberately uses one plain CSS file and no JavaScript or npm build. Replace it when a product needs a larger website instead of growing a second application framework inside the template.
...
3. Set the Rails application origin in `hugo.yaml` under `params.app_url`.
