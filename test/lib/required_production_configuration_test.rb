...
class RequiredProductionConfigurationTest < ActiveSupport::TestCase
  ...
  test "does not require S3 settings for another storage service" do
    environment = { "HOST" => "app.example.com", "DATABASE_URL" => "postgres://example", "ACTIVE_STORAGE_SERVICE" => "local" }
  ...
  test "reports all missing settings" do
    ...
    assert_equal "Missing required production configuration: HOST, DATABASE_URL, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, BUCKET_NAME", error.message
  end
end
